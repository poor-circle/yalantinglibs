...
Lazy<std::vector<std::chrono::microseconds>> call_echo(coro_io::client_pool<coro_rpc_client> &client_pool,
                     int cnt) {
  std::locale l;
  ...
  for (int i = 0; i < cnt; ++i) {
    ...
    if (!res) {
      ...
      result.push_back(std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now()-tp));
    }
  }
...
void latency_watcher() {
  ...
  for(auto e:arr) {
    std::cout<<(e*100)<<"% request finished in:"<<result[std::max<std::size_t>(0,result.size()*e-1)].count()/1000.0<<"ms"<<std::endl;
  }
}
int main() {
